...
export function initSorting(columns) {
    return (data, state, action) => {
        ...
        if (action && action.name === 'sort') {
            ...
            // Переключаем состояние нажатой кнопки по кругу: none -> up -> down -> none
            action.dataset.value = sortMap[action.dataset.value];    // Сохраним и применим как текущее следующее состояние из карты
            // Сразу читаем из неё обновленные данные для сортировки
        ...
        // Если сортировка выключена (order === 'none'), можно сразу вернуть данные
        if (order === 'none') {
            return data;
        }
